...
// TODO: добавить возможность перезаписывать значения, а не добавлять каждый раз новые

export default class universalDebugger {
	constructor(options = {}) {
		if (window.universalDebugger) {
			console.trace('Universal debugger already inited');
			return window.universalDebugger;
		}
		...
		if (!this.options.forceEnable && getQuery() && getQuery().debug != 1) {
			this.disabled = true;
			return;
		}
